Describe 'Get-NAVHighestVersionList' {
    BeforeAll {
        . "$PSScriptRoot\Get-NAVHighestVersionList.ps1"
    }
    ...
    It 'Shows warning if one or more version lists have a different prefix' {
        (Get-NAVHighestVersionList -VersionList1 'NAVW18.00' -VersionList2 'NAVNA9.00' -Prefix 'NAVW1' 3>&1) -match 'Versionlists are probably too unstructured to compare' | Should -BeTrue
        (Get-NAVHighestVersionList -VersionList1 'NAVW18.00' -VersionList2 'NAVBE9.00' -Prefix 'NAVNL' 3>&1) -match 'Versionlists are probably too unstructured to compare' | Should -BeTrue
    }
}